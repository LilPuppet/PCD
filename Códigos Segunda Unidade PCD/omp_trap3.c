...
int thread_assignments[1000]; // Global array to track thread assignments
...
# pragma omp parallel for num_threads(thread_count) \
   ...
   for (i = 1; i <= n-1; i++) {
     approx += f(a + i*h);
     thread_assignments[i] = omp_get_thread_num(); // Assign thread number
   }
   approx = h*approx; 
